...
import com.mihayvictor.labrary_spring.controller.exception.StandarError;
...
public class AuthorService {
    ...
    public Author findById(Long id){
        Optional<Author> obj = Optional.ofNullable(repository.findById(id)
                .orElseThrow(() -> new ObjectNotFoundExeception("Author não encontrado")));;
        return obj.get();
    }
    ...
    public void delete(Long id){
        try {
            Author author = repository.findById(id)
                    .orElseThrow(() -> new ObjectNotFoundExeception("Autor de ID " + id + " não encontrado"));
